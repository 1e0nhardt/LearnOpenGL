...
namespace LGL
{
    ...
    void Model::loadModel(std::string const& path)
    {
        ...
        {
            LGL_CORE_ERROR("ERROR::ASSIMP:: {0}", importer.GetErrorString());
            return;
        }
    ...
    std::vector<LGL::Texture2D> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
    {
        std::vector<Texture2D> textures;
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {   // if texture hasn't been loaded already, load it
                Texture2D texture(this->directory + "/" + std::string(str.C_Str()), typeName, false);
